...
目前此表是 snapshot table，永遠只保留最新一筆。程式寫入時會先清空表，再寫入本輪最新 STD AVG。
...
目前此表是 snapshot table，永遠只保留最新一筆。程式寫入時會先清空表，再寫入本輪最新 PORT AVG。
...
3. `STD_AVG` 與 `PORT_AVG` 是 snapshot table，目前永遠只保留最新一筆。
